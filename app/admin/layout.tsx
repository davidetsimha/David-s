...
export default function AdminLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <QueryProvider>
      <div className="min-h-screen bg-slate-50">
        {/* Mobile header */}
        <header className="lg:hidden fixed top-0 inset-x-0 h-14 bg-white border-b border-gray-100 flex items-center px-4 z-30 shadow-sm">
          <button
          ...
          <div className="flex items-center gap-2 ms-2">
            <div className="w-7 h-7 rounded-md bg-gradient-to-br from-amber-400 to-amber-600 flex items-center justify-center">
              <span className="font-serif text-white text-xs font-semibold">D</span>
            </div>
            <span className="font-serif text-gray-900 text-sm">David's Admin</span>
          </div>
        </header>
