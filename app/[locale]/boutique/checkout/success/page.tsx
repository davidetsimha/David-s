...
const POLLING_TIMEOUT = 60000; // 1 minute max
...
export default function CheckoutSuccessPage() {
  ...
  // Yaad Shrig renvoie CCode=0 pour succès (pas result=000)
  const ccode = searchParams.get('CCode');
  const paymentApproved = ccode === '0';
  const paymentDeclined = !!ccode && ccode !== '0';
  ...
  // Vide le panier dès que le paiement est confirmé
  useEffect(() => {
  ...
  // Vérification du statut côté API
  const checkPaymentStatus = useCallback(async () => {
    ...
    try {
      ...
      console.error('[CheckoutSuccess] Error polling status:', err);
    }
    return null;
  }, [orderId]);

  // Polling — instance unique, ne redémarre pas sur chaque changement d'état.
  // BUG CORRIGÉ : paymentStatus retiré des deps pour éviter les boucles parallèles.
  const pollingStarted = useRef(false);

  useEffect(() => {
    // Si déjà résolu (CCode=0 ou plateau) → aucun polling nécessaire
    if (paymentStatus !== 'loading' && paymentStatus !== 'pending') return;
    if (isPlateauOrder) { setPaymentStatus('confirmed'); return; }
    ...
    const maxPolls = Math.ceil(POLLING_TIMEOUT / POLLING_INTERVAL);
    ...
    const poll = async () => {
      ...
      if (pollCount >= maxPolls) { setPollingTimedOut(true); return; }
  ...
  }, []); // intentionnel — démarre une seule fois au montage
      ...
      <div className="max-w-md w-full text-center">
        ...
        {orderId && (
          <div className="bg-white rounded-xl p-5 border border-cream-200 mb-8">
            <p className="text-sm text-stone-500 mb-1">{t('checkout.orderNumber')}</p>
            <p className="font-display text-xl text-gold-700">#{orderId.slice(0, 8)}</p>
          </div>
        )}
        ...
        {isPlateauOrder && (
          <div className="bg-amber-50 rounded-xl p-4 border border-amber-200 mb-8">
            <p className="text-sm text-amber-800">{t('checkout.plateauOrderNote')}</p>
          </div>
        )}

        {/* Avis paiement échoué */}
        {showFailed && (
          <div className="bg-red-50 rounded-xl p-4 border border-red-200 mb-8">
            <p className="text-sm text-red-800">{t('checkout.paymentFailedNote')}</p>
          </div>
        )}
        ...
        {pollingTimedOut && (
          <div className="bg-amber-50 rounded-xl p-4 border border-amber-200 mb-8">
            <p className="text-sm text-amber-800">{t('checkout.errors.checkEmail')}</p>
          </div>
        )}
        ...
        <div className="flex items-center justify-center gap-3 text-sm text-stone-600 mb-8">
          ...
          <a href={`tel:${WHATSAPP_NUMBER.replace(/\s/g, '')}`}
            ...
            <>
              <Link
                ...
                className="flex items-center justify-center gap-2 w-full py-3.5
                  ...
                  shadow-md shadow-gold-500/20 hover:bg-gold-600 hover:shadow-lg
              ...
              <Link href="/"
                className="block w-full py-3 text-gold-700 hover:text-gold-800 font-medium transition-colors">
                {t('notFound.backHome')}
              </Link>
            </>
          ) : (
